...
public class NormalTask extends HbaseAbstractTask {
    ...
    public Put convertRecordToPut(Record record){
        ...
        for (Configuration aColumn : columns) {
            ...
            if(index >= record.getColumnNumber()){
                throw DataXException.asDataXException(Hbase23xWriterErrorCode.ILLEGAL_VALUE, String.format("您的column配置项中中index值超出范围,根据reader端配置,index的值小于%s,而您配置的值为%s，请检查并修改.",record.getColumnNumber(),index));
            }
    ...
    public byte[] getRowkey(Record record){
        ...
        for (Configuration aRowkeyColumn : rowkeyColumn) {
            ...
            if(index == -1){
                ...
                if(index >= record.getColumnNumber()){
                    throw DataXException.asDataXException(Hbase23xWriterErrorCode.CONSTRUCT_ROWKEY_ERROR, String.format("您的rowkeyColumn配置项中中index值超出范围,根据reader端配置,index的值小于%s,而您配置的值为%s，请检查并修改.",record.getColumnNumber(),index));
                }
    ...
    public long getVersion(Record record){
        ...
        if(index == -1){
            ...
            if(timestamp < 0){
                throw DataXException.asDataXException(Hbase23xWriterErrorCode.CONSTRUCT_VERSION_ERROR, "您指定的版本非法!");
            }
        }else{
            //指定列作为版本,long/doubleColumn直接record.aslong, 其它类型尝试用yyyy-MM-dd HH:mm:ss,yyyy-MM-dd HH:mm:ss SSS去format
            if(index >= record.getColumnNumber()){
                throw DataXException.asDataXException(Hbase23xWriterErrorCode.CONSTRUCT_VERSION_ERROR, String.format("您的versionColumn配置项中中index值超出范围,根据reader端配置,index的值小于%s,而您配置的值为%s，请检查并修改.",record.getColumnNumber(),index));
            }
            if(record.getColumn(index).getRawData()  == null){
                throw DataXException.asDataXException(Hbase23xWriterErrorCode.CONSTRUCT_VERSION_ERROR, "您指定的版本为空!");
            }
            ...
            if(record.getColumn(index) instanceof LongColumn || record.getColumn(index) instanceof DoubleColumn){
                ...
                try{
                    ...
                    try {
                        ...
                        throw DataXException.asDataXException(Hbase23xWriterErrorCode.CONSTRUCT_VERSION_ERROR, e1);
                    }
                }
