...
public class Hbase23xHelper {

    private static final Logger LOG = LoggerFactory.getLogger(Hbase23xHelper.class);
    ...
    public static org.apache.hadoop.conf.Configuration getHbaseConfiguration(String hbaseConfig) {
        if (StringUtils.isBlank(hbaseConfig)) {
            throw DataXException.asDataXException(Hbase23xWriterErrorCode.REQUIRED_VALUE, "读 Hbase 时需要配置hbaseConfig，其内容为 Hbase 连接信息，请联系 Hbase PE 获取该信息.");
        }
        ...
        try {
            ...
            throw DataXException.asDataXException(Hbase23xWriterErrorCode.GET_HBASE_CONNECTION_ERROR, e);
        }
        return hConfiguration;
    }

    public static org.apache.hadoop.hbase.client.Connection getHbaseConnection(String hbaseConfig) {
        org.apache.hadoop.conf.Configuration hConfiguration = Hbase23xHelper.getHbaseConfiguration(hbaseConfig);
        ...
        try {
            ...
            Hbase23xHelper.closeConnection(hConnection);
            throw DataXException.asDataXException(Hbase23xWriterErrorCode.GET_HBASE_CONNECTION_ERROR, e);
        }
        return hConnection;
    }
    ...
    public static Table getTable(com.alibaba.datax.common.util.Configuration configuration){
        ...
        org.apache.hadoop.hbase.client.Connection hConnection = Hbase23xHelper.getHbaseConnection(hbaseConfig);
        ...
        try {
            ...
            Hbase23xHelper.checkHbaseTable(admin,hTableName);
            ...
            Hbase23xHelper.closeTable(hTable);
            Hbase23xHelper.closeAdmin(admin);
            Hbase23xHelper.closeConnection(hConnection);
            throw DataXException.asDataXException(Hbase23xWriterErrorCode.GET_HBASE_TABLE_ERROR, e);
        }
        return hTable;
    }
    ...
    public static BufferedMutator getBufferedMutator(com.alibaba.datax.common.util.Configuration configuration){
        ...
        try {
            ...
            Hbase23xHelper.closeBufferedMutator(bufferedMutator);
            Hbase23xHelper.closeAdmin(admin);
            Hbase23xHelper.closeConnection(hConnection);
            throw DataXException.asDataXException(Hbase23xWriterErrorCode.GET_HBASE_BUFFEREDMUTATOR_ERROR, e);
        }
    ...
    public static void deleteTable(com.alibaba.datax.common.util.Configuration configuration) {
        ...
        Scan scan = new Scan();
        org.apache.hadoop.hbase.client.Table hTable = Hbase23xHelper.getTable(configuration);
        ...
        try {
            ...
            throw DataXException.asDataXException(Hbase23xWriterErrorCode.DELETE_HBASE_ERROR, e);
        }finally {
            if(scanner != null){
                scanner.close();
            }
    ...
    public static void truncateTable(com.alibaba.datax.common.util.Configuration configuration) {
        ...
        try{
            ...
            throw DataXException.asDataXException(Hbase23xWriterErrorCode.TRUNCATE_HBASE_ERROR, e);
        }finally {
            Hbase23xHelper.closeAdmin(admin);
            Hbase23xHelper.closeConnection(hConnection);
        }
    ...
    public static void closeConnection(Connection hConnection){
        try {
            ...
            throw DataXException.asDataXException(Hbase23xWriterErrorCode.CLOSE_HBASE_CONNECTION_ERROR, e);
        }
    }
    ...
    public static void closeAdmin(Admin admin){
        try {
            ...
            throw DataXException.asDataXException(Hbase23xWriterErrorCode.CLOSE_HBASE_AMIN_ERROR, e);
        }
    }
    ...
    public static void closeBufferedMutator(BufferedMutator bufferedMutator){
        try {
            ...
            throw DataXException.asDataXException(Hbase23xWriterErrorCode.CLOSE_HBASE_BUFFEREDMUTATOR_ERROR, e);
        }
    }
    ...
    public static void closeTable(Table table){
        try {
            ...
            throw DataXException.asDataXException(Hbase23xWriterErrorCode.CLOSE_HBASE_TABLE_ERROR, e);
        }
    }
    ...
    private static  void checkHbaseTable(Admin admin,  TableName hTableName) throws IOException {
        if(!admin.tableExists(hTableName)){
            throw DataXException.asDataXException(Hbase23xWriterErrorCode.ILLEGAL_VALUE, "HBase源头表" + hTableName.toString()
    ...
    public static void validateParameter(com.alibaba.datax.common.util.Configuration originalConfig) {
        originalConfig.getNecessaryValue(Key.HBASE_CONFIG, Hbase23xWriterErrorCode.REQUIRED_VALUE);
        originalConfig.getNecessaryValue(Key.TABLE, Hbase23xWriterErrorCode.REQUIRED_VALUE);

        Hbase23xHelper.validateMode(originalConfig);

        String encoding = originalConfig.getString(Key.ENCODING, Constant.DEFAULT_ENCODING);
        if (!Charset.isSupported(encoding)) {
            throw DataXException.asDataXException(Hbase23xWriterErrorCode.ILLEGAL_VALUE, String.format("HBaseWriter 不支持您所配置的编码:[%s]", encoding));
        }
    ...
    private  static  void validateMode(com.alibaba.datax.common.util.Configuration originalConfig){
        String mode = originalConfig.getNecessaryValue(Key.MODE, Hbase23xWriterErrorCode.REQUIRED_VALUE);
        ...
        switch (modeType) {
            ...
            default:
                throw DataXException.asDataXException(Hbase23xWriterErrorCode.ILLEGAL_VALUE,
                        String.format("HBaseWriter不支持该 mode 类型:%s", mode));
        }
    }
    ...
    private static void validateColumn(com.alibaba.datax.common.util.Configuration originalConfig){
        ...
        if (columns == null || columns.isEmpty()) {
            throw DataXException.asDataXException(Hbase23xWriterErrorCode.REQUIRED_VALUE, "column为必填项，其形式为：column:[{\"index\": 0,\"name\": \"cf0:column0\",\"type\": \"string\"},{\"index\": 1,\"name\": \"cf1:column1\",\"type\": \"long\"}]");
        }
        for (Configuration aColumn : columns) {
            Integer index = aColumn.getInt(Key.INDEX);
            String type = aColumn.getNecessaryValue(Key.TYPE, Hbase23xWriterErrorCode.REQUIRED_VALUE);
            String name = aColumn.getNecessaryValue(Key.NAME, Hbase23xWriterErrorCode.REQUIRED_VALUE);
            ColumnType.getByTypeName(type);
            if(name.split(":").length != 2){
                throw DataXException.asDataXException(Hbase23xWriterErrorCode.ILLEGAL_VALUE, String.format("您column配置项中name配置的列格式[%s]不正确，name应该配置为 列族:列名  的形式, 如 {\"index\": 1,\"name\": \"cf1:q1\",\"type\": \"long\"}", name));
            }
            if(index == null || index < 0){
                throw DataXException.asDataXException(Hbase23xWriterErrorCode.ILLEGAL_VALUE, "您的column配置项不正确,配置项中中index为必填项,且为非负数，请检查并修改.");
            }
        }
    }
    ...
    private static void validateRowkeyColumn(com.alibaba.datax.common.util.Configuration originalConfig){
        ...
        if (rowkeyColumn == null || rowkeyColumn.isEmpty()) {
            throw DataXException.asDataXException(Hbase23xWriterErrorCode.REQUIRED_VALUE, "rowkeyColumn为必填项，其形式为：rowkeyColumn:[{\"index\": 0,\"type\": \"string\"},{\"index\": -1,\"type\": \"string\",\"value\": \"_\"}]");
        }
        ...
        for (Configuration aRowkeyColumn : rowkeyColumn) {
            ...
            String type = aRowkeyColumn.getNecessaryValue(Key.TYPE, Hbase23xWriterErrorCode.REQUIRED_VALUE);
            ColumnType.getByTypeName(type);
            if(index == null ){
                throw DataXException.asDataXException(Hbase23xWriterErrorCode.REQUIRED_VALUE, "rowkeyColumn配置项中index为必填项");
            }
            //不能只有-1列,即rowkey连接串
            if(rowkeyColumnSize ==1 && index == -1){
                throw DataXException.asDataXException(Hbase23xWriterErrorCode.ILLEGAL_VALUE, "rowkeyColumn配置项不能全为常量列,至少指定一个rowkey列");
            }
            if(index == -1){
                aRowkeyColumn.getNecessaryValue(Key.VALUE, Hbase23xWriterErrorCode.REQUIRED_VALUE);
            }
        }
    }
    ...
    private static void validateVersionColumn(com.alibaba.datax.common.util.Configuration originalConfig){
        ...
        if(versionColumn != null){
            ...
            if(index == null ){
                throw DataXException.asDataXException(Hbase23xWriterErrorCode.REQUIRED_VALUE, "versionColumn配置项中index为必填项");
            }
            if(index == -1){
                //指定时间,需要index=-1,value
                versionColumn.getNecessaryValue(Key.VALUE, Hbase23xWriterErrorCode.REQUIRED_VALUE);
            }else if(index < 0){
                throw DataXException.asDataXException(Hbase23xWriterErrorCode.ILLEGAL_VALUE, "您versionColumn配置项中index配置不正确,只能取-1或者非负数");
            }
        }
    }
}
