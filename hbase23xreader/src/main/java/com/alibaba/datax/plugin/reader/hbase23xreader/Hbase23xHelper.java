...
public class Hbase23xHelper {
    ...
    public static Connection getHbaseConnection(String hbaseConfig) {
        if (StringUtils.isBlank(hbaseConfig)) {
            throw DataXException.asDataXException(Hbase23xReaderErrorCode.REQUIRED_VALUE, "读 Hbase 时需要配置hbaseConfig，其内容为 HBase 连接信息，请联系 HBase PE 获取该信息.");
        }
        ...
        try {
            ...
            throw DataXException.asDataXException(Hbase23xReaderErrorCode.GET_HBASE_CONNECTION_ERROR, e);
        }
    ...
    public static Table getTable(Configuration configuration){
        ...
        Connection hConnection = Hbase23xHelper.getHbaseConnection(hbaseConfig);
        ...
        try {
            ...
            Hbase23xHelper.closeConnection(hConnection);
            throw DataXException.asDataXException(Hbase23xReaderErrorCode.GET_HBASE_TABLE_ERROR, e);
        }
        return hTable;
    }
   ...
   public static RegionLocator getRegionLocator(Configuration configuration){
       ...
       try {
           ...
           Hbase23xHelper.closeRegionLocator(regionLocator);
           Hbase23xHelper.closeAdmin(admin);
           Hbase23xHelper.closeConnection(hConnection);
           throw DataXException.asDataXException(Hbase23xReaderErrorCode.GET_HBASE_REGINLOCTOR_ERROR, e);
       }
    ...
    public static void closeConnection(Connection hConnection){
        try {
            ...
            throw DataXException.asDataXException(Hbase23xReaderErrorCode.CLOSE_HBASE_CONNECTION_ERROR, e);
        }
    }
    ...
    public static void closeAdmin(Admin admin){
        try {
            ...
            throw DataXException.asDataXException(Hbase23xReaderErrorCode.CLOSE_HBASE_ADMIN_ERROR, e);
        }
    }
    ...
    public static void closeTable(Table table){
        try {
            ...
            throw DataXException.asDataXException(Hbase23xReaderErrorCode.CLOSE_HBASE_TABLE_ERROR, e);
        }
    }
    ...
    public static void closeRegionLocator(RegionLocator regionLocator){
        try {
            ...
            throw DataXException.asDataXException(Hbase23xReaderErrorCode.CLOSE_HBASE_REGINLOCTOR_ERROR, e);
        }
    }
    ...
    public static  void checkHbaseTable(Admin admin,  TableName hTableName) throws IOException {
        if(!admin.tableExists(hTableName)){
            throw DataXException.asDataXException(Hbase23xReaderErrorCode.ILLEGAL_VALUE, "HBase源头表" + hTableName.toString()
    ...
    public static byte[] convertUserStartRowkey(Configuration configuration) {
        ...
        if (StringUtils.isBlank(startRowkey)) {
            ...
            return Hbase23xHelper.stringToBytes(startRowkey, isBinaryRowkey);
        }
    }
    ...
    public static byte[] convertUserEndRowkey(Configuration configuration) {
        ...
        if (StringUtils.isBlank(endRowkey)) {
            ...
            boolean isBinaryRowkey = configuration.getBool(Key.IS_BINARY_ROWKEY);
            return Hbase23xHelper.stringToBytes(endRowkey, isBinaryRowkey);
        }
    }
    ...
    public static List<HbaseColumnCell> parseColumnOfNormalMode(List<Map> column) {
        ...
        for (Map<String, String> aColumn : column) {
            ...
            if (type == ColumnType.DATE) {
                ...
                Validate.isTrue(StringUtils.isNotBlank(columnName) || StringUtils.isNotBlank(columnValue), "HBaseReader 在 normal 方式读取时，其列配置中，如果类型不是时间，则要么是 type + name 的组合，要么是type + value 的组合. " +
                                "而您的配置非这两种组合，请检查并修改.");
    ...
    public static HashMap<String,HashMap<String,String>> parseColumnOfMultiversionMode(List<Map> column){
        ...
        for (Map<String, String> aColumn : column) {
            ...
            if( !Hbase23xHelper.isRowkeyColumn(columnName)){
                String[] cfAndQualifier = columnName.split(":");
                if ( cfAndQualifier.length != 2) {
                    throw DataXException.asDataXException(Hbase23xReaderErrorCode.ILLEGAL_VALUE, "HBaseReader 中，column 的列配置格式应该是：列族:列名. 您配置的列错误：" + columnName);
                }
    ...
    public static List<Configuration> split(Configuration configuration) {
        byte[] startRowkeyByte = Hbase23xHelper.convertUserStartRowkey(configuration);
        byte[] endRowkeyByte = Hbase23xHelper.convertUserEndRowkey(configuration);
        ...
        if (startRowkeyByte.length != 0 && endRowkeyByte.length != 0
            ...
            throw DataXException.asDataXException(Hbase23xReaderErrorCode.ILLEGAL_VALUE, "HBaseReader 中 startRowkey 不得大于 endRowkey.");
        }
        RegionLocator regionLocator = Hbase23xHelper.getRegionLocator(configuration);
        List<Configuration> resultConfigurations ;
        try {
            Pair<byte[][], byte[][]> regionRanges = regionLocator.getStartEndKeys();
            if (null == regionRanges) {
                throw DataXException.asDataXException(Hbase23xReaderErrorCode.SPLIT_ERROR, "获取源头 HBase 表的 rowkey 范围失败.");
            }
            resultConfigurations = Hbase23xHelper.doSplit(configuration, startRowkeyByte, endRowkeyByte,
                    regionRanges);

            LOG.info("HBaseReader split job into {} tasks.", resultConfigurations.size());
            return resultConfigurations;
        } catch (Exception e) {
            throw DataXException.asDataXException(Hbase23xReaderErrorCode.SPLIT_ERROR, "切分源头 HBase 表失败.", e);
        }finally {
            Hbase23xHelper.closeRegionLocator(regionLocator);
        }
    }
    ...
    public static void validateParameter(Configuration originalConfig) {
        originalConfig.getNecessaryValue(Key.HBASE_CONFIG, Hbase23xReaderErrorCode.REQUIRED_VALUE);
        originalConfig.getNecessaryValue(Key.TABLE, Hbase23xReaderErrorCode.REQUIRED_VALUE);
        ...
        if (!Charset.isSupported(encoding)) {
            throw DataXException.asDataXException(Hbase23xReaderErrorCode.ILLEGAL_VALUE, String.format("HBaseReader 不支持您所配置的编码:[%s]", encoding));
        }
    ...
    private static String validateMode(Configuration  originalConfig) {
        String mode = originalConfig.getNecessaryValue(Key.MODE, Hbase23xReaderErrorCode.REQUIRED_VALUE);
        List<Map> column = originalConfig.getList(Key.COLUMN, Map.class);
        if (column == null || column.isEmpty()) {
            throw DataXException.asDataXException(Hbase23xReaderErrorCode.REQUIRED_VALUE, "您配置的column为空,HBase必须配置 column，其形式为：column:[{\"name\": \"cf0:column0\",\"type\": \"string\"},{\"name\": \"cf1:column1\",\"type\": \"long\"}]");
        }
        ...
        switch (modeType) {
            case Normal: {
                ...
                Hbase23xHelper.parseColumnOfNormalMode(column);
                break;
            }
            case MultiVersionFixedColumn:{
                ...
                Hbase23xHelper.parseColumnOfMultiversionMode(column);
                break;
            }
            default:
                throw DataXException.asDataXException(Hbase23xReaderErrorCode.ILLEGAL_VALUE,
    ...
    private static void checkMaxVersion(Configuration configuration, String mode) {
        ...
        Validate.isTrue(isMaxVersionValid, String.format("您配置的是 %s 模式读取 HBase 中的数据，但是配置的 maxVersion 值错误. " +
                "maxVersion规定：-1为读取全部版本，不能配置为0或者1（因为0或者1，我们认为用户是想用 normal 模式读取数据，而非 %s 模式读取，二者差别大），大于1则表示读取最新的对应个数的版本", mode, mode));
    }
}
