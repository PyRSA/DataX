# Hbase094XWriter & Hbase11XWriter & Hbase23XWriter 插件文档
...
1、目前HbaseWriter支持的Hbase版本有：Hbase0.94.x和Hbase1.1.x、Hbase2.3.x。
...
* 若您的hbase版本为Hbase2.3.x，writer端的插件请选择：hbase23xwriter,即：
...
简而言之，HbaseWriter 通过 HBase 的 Java 客户端，通过 HTable, Put等 
API，将从上游Reader读取的数据写入HBase你hbase23xwriter与hbase094xwriter和hbase11xwriter的主要不同在于API的调用不同，Hbase2.3.x废弃了很多Hbase1.1.x之前的api。
    ...
    "content": [
      {
        "reader": {
          "name": "txtfilereader",
          "parameter": {
            "path": "/opt/datax/datax_test/hbase11xwriter/txt/normal.txt",
            "charset": "UTF-8",
            "column": [
              {
                "index": 0,
