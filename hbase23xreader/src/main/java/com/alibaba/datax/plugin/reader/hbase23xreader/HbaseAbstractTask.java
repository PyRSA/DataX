...
public abstract class HbaseAbstractTask {
    ...
    public HbaseAbstractTask(com.alibaba.datax.common.util.Configuration configuration) {
        ...
        this.htable = Hbase23xHelper.getTable(configuration);

        this.encoding = configuration.getString(Key.ENCODING,Constant.DEFAULT_ENCODING);
        this.startKey = Hbase23xHelper.convertInnerStartRowkey(configuration);
        this.endKey =  Hbase23xHelper.convertInnerEndRowkey(configuration);
    ...
    public void close()  {
        Hbase23xHelper.closeResultScanner(this.resultScanner);
        Hbase23xHelper.closeTable(this.htable);
    }
    ...
    public Column convertBytesToAssignType(ColumnType columnType, byte[] byteArray,String dateformat) throws Exception {
        ...
        switch (columnType) {
            ...
            case DATE:
                ...
                break;
            default:
                throw DataXException.asDataXException(Hbase23xReaderErrorCode.ILLEGAL_VALUE, "HBaseReader 不支持您配置的列类型:" + columnType);
        }
        return column;
    }
    ...
    public Column convertValueToAssignType(ColumnType columnType, String constantValue,String dateformat) throws Exception {
        ...
        switch (columnType) {
            ...
            case DATE:
                ...
                break;
            default:
                throw DataXException.asDataXException(Hbase23xReaderErrorCode.ILLEGAL_VALUE, "HBaseReader 常量列不支持您配置的列类型:" + columnType);
        }
        return column;
    }
}
